...
1. In the **main** module, one of the first lines of code is `const drovers = hireDrovers(cattleToDrive)`. Explain what the value of the `drovers` variable is when that line of code runs.
   ...
   > The invocation of the `const drovers = hireDrovers(cattleToDrive)` function declares the 'drovers' variable(s). When the code runs, it intitializes the amount of drovers(cattle staff) that will be on the journey, driving the cattle.
